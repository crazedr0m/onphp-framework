<?php
	...
	final class FormTest extends TestCase
	{
		...
		public function testErrors()
		{
			...
			//empty import
			$form->import([])->checkRules();

			//checking
			$expectingErrors = ['old' => Form::MISSING, 'someRule' => Form::WRONG];
			...
			//import wrong data
			$form->clean()->importMore(['flag' => '3', 'old' => '17'])->checkRules();

			//checking
			$expectingErrors = ['flag' => Form::WRONG, 'someRule' => Form::WRONG];
			$this->assertEquals($expectingErrors, $form->getErrors());
			$this->assertTrue($form->hasError('someRule'));

			//marking good and custom check errors
			$form->markGood('someRule')->markCustom('flag', 3);
			$this->assertEquals(['flag' => 3], $form->getErrors());
