<?php
	...
	class DataDBTest extends TestCaseDAO
	{
		...
		public function getListByIdsTest()
		{
			...
			$list = TestUser::dao()->getListByIds([1, 3, 2, 1, 1, 1]);
			...
			$this->assertEquals(
				[],
				TestUser::dao()->getListByIds([42, 42, 1738])
			);
		}
		...
		private function nonIntegerIdentifier()
		{
			...
			$this->assertTrue($prm->import(['id' => $id]));
			...
			try {
				$integerIdPrimitive->import(['id' => 'string-instead-of-integer']);
			} catch (DatabaseException $e) {
				return $this->fail($e->getMessage());
			}
		}
